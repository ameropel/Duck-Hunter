...
public class SplineInterpolator_Menu : MonoBehaviour
{
	// Script Holder
 	[HideInInspector] public AIManager_Menu sc_AIManager_Menu;
	...
	void Update()
	{
		...
		if (!ReachedQuaterTime)
		{
			...
			{				
				...
				sc_AIManager_Menu.Add_AIPath(gameObject.GetComponent<SplineController_Menu>().SplineRoot);
			}
		}
	...
	void OnDisable()
	{
		Destroy(gameObject.GetComponent<SplineController_Menu>().SplineRoot);
	}
}